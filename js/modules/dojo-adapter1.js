require([
	...
	'dojo/on'
			...
			aspect, domConstruct, domGeometry, domStyle, on){
...
var _Adapter = {
	...
	onAnimBeforeBegin:function(el, properties){
		...
		for(var p in properties){
			...
			prop = pm[p] = lang.mixin({}, (lang.isObject(prop) ? prop: { end: prop }));
	...
	onAnimEnd:function(el, callback){
		...
		delete ns.signals["onAnimateSignal"];
	...
	getScript: function (scriptLocation, callback) {
		...
		_Adapter._scriptSignals = [
			on(script,'load', lang.hitch(_Adapter,'onScriptLoaded', callback)),
			on(script,'readystatechange', lang.hitch(_Adapter,'onScriptLoaded', callback))
		];
	...
	onScriptLoaded:function(callback, e){
		...
		for(var i=0;i<_Adapter._scriptSignals.length;i++){
	...
	fireEvent: function (el, event, eventArguments, defaultFunction) {
		...
		var event = on.emit(el, event, eventArgs);
